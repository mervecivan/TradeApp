  ...
  if (loading) {
    return (
      <div className="bg-gray-900 rounded-xl shadow-lg border border-gray-800 p-6 flex items-center justify-center h-96">
        ...
        <span className="text-white">Loading Live Crypto Data...</span>
      </div>
    );
  }
    ...
    <div className="bg-gray-900 rounded-xl shadow-lg border border-gray-800 p-6">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Update {new Date().toLocaleDateString()} at {updateTime}
          </p>
        </div>
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {watchlistDisplay.map((item, index) => {
              ...
              const valueUSD = (currentPrice * quantity).toFixed(2);

              const volume = item.total_volume ? (item.total_volume / 1000000).toFixed(2) : "0";

              return (
                <TableRow key={item.id ?? item.symbol ?? index} className="border-gray-800 hover:bg-gray-800/70">
                  <TableCell className="font-medium flex items-center space-x-3 py-4">
                    ...
                    <div>
                      <p className="text-white text-sm">{item.symbol ?? "N/A"}</p>
                      <p className="text-xs text-gray-400">{item.name ?? "N/A"}</p>
                    </div>
                  </TableCell>
                  <TableCell className="text-white">${currentPrice.toFixed(2)}</TableCell>
                  <TableCell className={cn("font-medium", isUp ? "text-green-500" : "text-red-500")}>
                    <div className="flex items-center">
                      {isUp ? <ArrowUp className="w-3 h-3 mr-1" /> : <ArrowDown className="w-3 h-3 mr-1" />}
                      ${valueUSD}
                    </div>
                  </TableCell>
                  <TableCell className="text-white">${volume}M</TableCell>
                  <TableCell>
                    <svg width="60" height="20" viewBox="0 0 60 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path
                        d={isUp ? "M1 18L10 10L20 15L30 5L40 12L50 8L59 15" : "M1 5L10 13L20 8L30 18L40 11L50 15L59 8"}
